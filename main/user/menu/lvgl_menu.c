...
static void create_block_top_left(ui_main_menu_t *ui) {
	...
	value = create_label(
		ui->sensor.screen, "0 ppm", BLOCK_TOP_LEFT_ALIGN_VALUES,
		BLOCK_TOP_LEFT_X_START_VALUES, BLOCK_TOP_LEFT_Y_START_VALUE_3);
	if (!value)
		return;
...
static void create_block_top_right(ui_main_menu_t *ui) {
	...
	time = create_label(
		ui->time.screen, "load...", BLOCK_TOP_RIGHT_ALIGN_VALUES,
		BLOCK_TOP_RIGHT_X_START_VALUE_3, BLOCK_TOP_RIGHT_Y_START_VALUE_3);
	if (!time)
		return;
...
static void ui_create_weather_popup(ui_main_menu_t *ui) {
	...
	lv_obj_set_style_bg_img_src(ui->weather.btn_open_city_list, LV_SYMBOL_GPS,
								0);
...
static void ui_create_settings_popup_btns(ui_main_menu_t *ui) {
	...
	ui->settings.btn_open = create_btn_cb(
		ui->screen, BLOCK_BOT_MID_WIDTH_SYMBOL, BLOCK_BOT_MID_HEIGHT_SYMBOL,
		BLOCK_BOT_MID_ALIGN_SYMBOL, BLOCK_BOT_MID_X_START_SYMBOL_4,
...
static void create_block_bot_right(ui_main_menu_t *ui) {
	...
	for (uint8_t i = 0; i < BLOCK_BOT_RIGHT_MAX_WEATHER_ANIM_SNOWS; i++) {
		ui->animation.snow[i] =
			create_rain_snow_anim(&snow_flake_2_15_15, ui->animation.screen, 2,
								  BLOCK_BOT_RIGHT_SPEED_WEATHER_ANIM_RAIN_SNOW);
...
static void create_menu(ui_main_menu_t *ui) {
	...
	//  lv_obj_set_style_bg_color(lv_object[BACKGROUND_MAIN_MENU],
	//  lv_color_black(),
	//                            0);
	//  lv_obj_set_style_bg_opa(lv_object[BACKGROUND_MAIN_MENU], LV_OPA_COVER,
	//  0);
...
void rain_set_intensity(uint8_t visible_count, ui_main_menu_t *ui) {
	...
	if (visible_count > prev) {
		for (uint8_t i = prev; i < visible_count; i++)
			lv_obj_clear_flag(ui->animation.rain[i], LV_OBJ_FLAG_HIDDEN);
	} else {
		for (uint8_t i = visible_count; i < prev; i++)
...
void draw_symbol_wifi(ui_main_menu_t *ui) {
	...
	if (curent_status != status_wifi_old) {
		...
		case WIFI_RECONNECT:
			...
			lv_obj_set_style_bg_img_src(ui->wifi.btn_open, LV_SYMBOL_REFRESH,
										0);
			break;
		case WIFI_CONNECTED:
		...
		case WIFI_DISCONNECTED:
			lv_obj_set_style_bg_img_src(ui->wifi.btn_open, LV_SYMBOL_WARNING,
										0);
			break;
		default:
			lv_obj_set_style_bg_img_src(ui->wifi.btn_open, LV_SYMBOL_WARNING,
