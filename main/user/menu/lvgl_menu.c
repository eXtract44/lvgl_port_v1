...
static lv_obj_t *create_meter(lv_obj_t *parent, lv_coord_t w, lv_coord_t h,
	...
	ui->co2.indicator = lv_meter_add_scale_lines(
...
void print_time(uint8_t time_hour, uint8_t time_minute, ui_main_menu_t *ui) {
	...
	if (get_wifi_status() == WIFI_CONNECTED) {
		if (time_hour < 10 && time_minute < 10) {
			sprintf(ui->string_buffer, "0%d:0%d", (int)time_hour,
					(int)time_minute);
		} else if (time_hour > 9 && time_minute < 10) {
			sprintf(ui->string_buffer, "%d:0%d", (int)time_hour,
					(int)time_minute);
		} else if (time_hour < 10 && time_minute > 9) {
			sprintf(ui->string_buffer, "0%d:%d", (int)time_hour,
					(int)time_minute);
		} else {
			sprintf(ui->string_buffer, "%d:%d", (int)time_hour,
					(int)time_minute);
		}
	} else {
...
void print_mday(uint8_t date_day, uint8_t date_month, ui_main_menu_t *ui) {
	...
	if (get_wifi_status() == WIFI_CONNECTED) {
		if (date_day < 10 && date_month < 10) {
			sprintf(ui->string_buffer, "0%d.0%d", (int)date_day,
					(int)date_month);
		} else if (date_day > 9 && date_month < 10) {
			sprintf(ui->string_buffer, "%d.0%d", (int)date_day,
					(int)date_month);
		} else if (date_day < 10 && date_month > 9) {
			sprintf(ui->string_buffer, "0%d.%d", (int)date_day,
					(int)date_month);
		} else {
...
static void wind_anim_cb(void *var, int32_t v) {
	...
	lv_coord_t y = w->base_y + (lv_trigo_sin(sin_arg) * w->turbulence) /
								   LV_TRIGO_SIN_MAX / 3;
...
static void block_top_left_open_popup_event_handler(lv_event_t *e) {
	...
	if (ui->sensor.popup.popup != NULL &&
		lv_obj_is_valid(ui->sensor.popup.popup))
		return;
...
static void ui_create_sensor_history_popup(ui_main_menu_t *ui) {
	...
	ui->sensor.popup.btn_close =
		create_btn_cb(ui->sensor.popup.popup, 50, 50, LV_ALIGN_TOP_RIGHT, -5,
					  -5, btn_sensor_close_history_popup_event_handler, ui);
...
static void create_block_bot_left(ui_main_menu_t *ui) {
	/*BLOCK BOT LEFT*/
	lv_obj_t *bg = create_background(
		ui->screen, BLOCK_BOT_LEFT_WIDTH, BLOCK_BOT_LEFT_HEIGHT,
	...
	lv_obj_add_event_cb(ui->weather.screen,
						block_bot_left_open_popup_event_handler,
						LV_EVENT_CLICKED, ui);
	...
	lv_obj_t *icon = create_icon(
		ui->weather.screen, BLOCK_BOT_LEFT_WIDTH_SYMBOLS,
	...
	value = create_label(
		ui->weather.screen, "0 m/s", BLOCK_BOT_LEFT_ALIGN_VALUES,
		BLOCK_BOT_LEFT_X_START_VALUES, BLOCK_BOT_LEFT_Y_START_VALUE_3);
	if (!value) {
		return;
	}
	lv_obj_add_style(value, &ui->font.small, 0);
	...
	if (lv_obj_is_valid(ui->weather.settings_popup.city_label)) {
		lv_label_set_text(ui->weather.settings_popup.city_label,
						  ui->weather.settings_popup
							  ...
							  .name);
		lv_obj_add_style(ui->weather.settings_popup.city_label, &ui->font.small,
						 0);
	}
}
...
static void ui_create_co2(ui_main_menu_t *ui) {
	...
	ui->co2.chart = create_chart(ui->screen, BLOCK_BOT_MID_WIDTH_CO2_CHART,
...
static void ui_create_weather_settings_popup(ui_main_menu_t *ui) {
	...
	ui->weather.settings_popup.btn_close = create_btn_cb(
		ui->weather.settings_popup.popup, 50, 50, LV_ALIGN_TOP_LEFT, 500, -10,
	...
	ui->weather.settings_popup.btn_open_city_list = create_btn_cb(
		ui->weather.settings_popup.popup, 50, 50, LV_ALIGN_TOP_LEFT, 500, 90,
...
static void ui_create_wifi_popup(ui_main_menu_t *ui) {
	...
	create_text("Name:", ui->wifi.popup, STYLE_TEXT_SMALL, LV_ALIGN_TOP_LEFT,
				15, 90, ui);
	create_text("Passwort:", ui->wifi.popup, STYLE_TEXT_SMALL,
...
void update_block_bot_middle(ui_main_menu_t *ui) {
	...
	if (cnt_chart_co2 > 3600) { // equal 1 hour
		...
		lv_chart_set_next_value(ui->co2.chart, ui->co2.series_co2,
								temp_co2_chart);
		cnt_chart_co2 = 0;
	}
	draw_symbol_wifi(ui);
