s#include "user/menu/lvgl_menu.h"
...
// void set_visible(lv_obj_t *obj, bool f) {
//   if (f)
//     lv_obj_clear_flag(obj, LV_OBJ_FLAG_HIDDEN); // on
//   else
//     lv_obj_add_flag(obj, LV_OBJ_FLAG_HIDDEN); // off
...
static lv_obj_t *create_meter(lv_obj_t *screen, lv_coord_t w, lv_coord_t h,
	...
	indicator[INDICATOR_CO2] = lv_meter_add_scale_lines(
		meter, scale, lv_palette_main(LV_PALETTE_GREEN),
...
static void create_block_top_left() {
	...
	lv_object[BACKGROUND_BLOCK_TOP_LEFT] = create_background(
		lv_object[SCREEN_MAIN_MENU], BLOCK_TOP_LEFT_WIDTH,
		BLOCK_TOP_LEFT_HEIGHT, BLOCK_TOP_LEFT_ALIGN_BACKGROUND,
		BLOCK_TOP_LEFT_X_START, BLOCK_TOP_LEFT_Y_START);
	lv_obj_add_style(lv_object[BACKGROUND_BLOCK_TOP_LEFT], &style_bg_top_left,
					 0);

	create_text("inside", lv_object[BACKGROUND_BLOCK_TOP_LEFT],
				STYLE_TEXT_SMALL, BLOCK_TOP_LEFT_ALIGN_TITLE, 0,
				BLOCK_TOP_LEFT_Y_START_TITLE);
	...
	lv_obj_set_style_text_font(lv_object[SYMBOL_TEMPERATURE_INSIDE],
							   &my_symbols, 0);
	...
	lv_obj_set_style_text_font(lv_object[SYMBOL_HUMIDITY_INSIDE], &my_symbols,
							   0);
	...
	lv_object[VALUE_TEMPERATURE_INSIDE] =
		create_label(lv_object[BACKGROUND_BLOCK_TOP_LEFT], "0 c*",
					 BLOCK_TOP_LEFT_ALIGN_VALUES, BLOCK_TOP_LEFT_X_START_VALUES,
					 BLOCK_TOP_LEFT_Y_START_VALUE_1);
	lv_object[VALUE_HUMIDITY_INSIDE] =
		create_label(lv_object[BACKGROUND_BLOCK_TOP_LEFT], "0 %*",
					 BLOCK_TOP_LEFT_ALIGN_VALUES, BLOCK_TOP_LEFT_X_START_VALUES,
...
static void create_block_top_right() {
	...
	lv_obj_add_style(lv_object[BACKGROUND_CLOCK_VALUES], &style_bg_top_right,
					 0);
	...
	lv_object[TIME_HOUR_MINUTE] = create_label(
		lv_object[BACKGROUND_CLOCK_VALUES], "00:00",
		BLOCK_TOP_RIGHT_ALIGN_VALUES, BLOCK_TOP_RIGHT_X_START_VALUES,
		BLOCK_TOP_RIGHT_Y_START_VALUE_1);
...
static void create_block_bot_left() {
	...
	lv_object[BACKGROUND_BLOCK_BOT_LEFT] = create_background(
		lv_object[SCREEN_MAIN_MENU], BLOCK_BOT_LEFT_WIDTH,
		BLOCK_BOT_LEFT_HEIGHT, BLOCK_BOT_LEFT_ALIGN_BACKGROUND,
		BLOCK_BOT_LEFT_X_START, BLOCK_BOT_LEFT_Y_START);
	lv_obj_add_style(lv_object[BACKGROUND_BLOCK_BOT_LEFT], &style_bg_bot_left,
					 0);
	create_text("outside", lv_object[BACKGROUND_BLOCK_BOT_LEFT],
				STYLE_TEXT_SMALL, BLOCK_BOT_LEFT_ALIGN_TITLE, 0,
				BLOCK_BOT_LEFT_Y_START_TITLE);
	...
	lv_obj_set_style_text_font(lv_object[SYMBOL_TEMPERATURE_OUTSIDE],
							   &my_symbols, 0);
...
static void create_window_popup(char *window_name, lv_obj_t *bg) {
	...
	// lv_obj_add_style(lv_object[BACKGROUND_POPUP_WIFI], &style_bg_bot_right,
	// 0);
...
static void create_block_bot_right() {
	...
	lv_object[IMAGE_CLOUD_MID_90_45] = create_cloud_anim(
		&cloud_mid_90_45, lv_object[BACKGROUND_WEATHER],
...
static void create_menu() {
	...
	lv_object[BACKGROUND_MAIN_MENU] =
		lv_obj_create(lv_object[SCREEN_MAIN_MENU]);
...
void update_block_top_left() {
	...
	print_value("%.1f c*", get_temperature_aht10(),
				lv_object[VALUE_TEMPERATURE_INSIDE]);
	print_value("%.f %%", get_humidity_aht10(),
				lv_object[VALUE_HUMIDITY_INSIDE]);
...
static void init_fonts() {
	...
	lv_style_set_text_font(&style[STYLE_TEXT_VERY_LARGE],
						   &lv_font_montserrat_48);
}
