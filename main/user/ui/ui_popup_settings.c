...
void ui_create_settings_popup(ui_main_menu_t *ui) {
	...
	lv_obj_set_style_bg_color(ui->settings.page_home, lv_color_hex(0x888888),
							  LV_PART_SCROLLBAR);
	lv_obj_set_style_bg_opa(ui->settings.page_home, LV_OPA_70,
							LV_PART_SCROLLBAR);
	...
	lv_obj_set_scrollbar_mode(ui->settings.page_content,
							  LV_SCROLLBAR_MODE_AUTO);
	lv_obj_set_style_bg_color(ui->settings.page_content, lv_color_hex(0x888888),
							  LV_PART_SCROLLBAR);
	lv_obj_set_style_bg_opa(ui->settings.page_content, LV_OPA_70,
							LV_PART_SCROLLBAR);
	...
	for (int i = 0; i < SETTINGS_PAGE_LAST_ELEMENT - 1; i++) {
		...
		lv_obj_set_style_bg_color(btn, lv_color_hex(0x2D6AB4),
								  LV_STATE_PRESSED);
...
void ui_create_settings_popup_btns(ui_main_menu_t *ui) {
	ui->settings.btn_open = create_btn_icon(
		...
		ui, LV_SYMBOL_SETTINGS, NULL, &lv_font_montserrat_32, "Einstellungen",
		ui);
}
...
void btn_settings_close_popup_event_handler(lv_event_t *e) {
	...
	ui->settings.backlight_min_slider = NULL;
	ui->settings.backlight_max_slider = NULL;
	ui->settings.backlight_min_label = NULL;
	ui->settings.backlight_max_label = NULL;
	...
	ui->settings.ota_last_update_label = NULL;
...
void btn_settings_category_event_handler(lv_event_t *e) {
	...
	settings_page_t page =
		(settings_page_t)(uintptr_t)lv_obj_get_user_data(btn);
	...
	case SETTINGS_PAGE_UPDATE:
		...
		create_text("Firmware Update", ui->settings.page_content,
					STYLE_TEXT_SMALL, LV_ALIGN_TOP_MID, 0, 10, ui);

		// --- Текущая версия ---
		create_text("Aktuelle Version:", ui->settings.page_content,
					STYLE_TEXT_SMALL, LV_ALIGN_TOP_LEFT, 10, 60, ui);
		ui->settings.ota_local_ver_label =
			create_label(ui->settings.page_content, ota_get_current_version(),
						 LV_ALIGN_TOP_LEFT, 300, 60);
		lv_obj_add_style(ui->settings.ota_local_ver_label, &ui->font.medium_32,
						 0);
		...
		lv_obj_add_style(ui->settings.ota_remote_ver_label, &ui->font.medium_32,
						 0);

		create_text("Letztes Update:", ui->settings.page_content,
					STYLE_TEXT_SMALL, LV_ALIGN_TOP_LEFT, 10, 160, g_ui);

		// --- Кнопка Pruefen ---
		ui->settings.ota_check_btn =
			create_btn_cb(ui->settings.page_content, 200, 60, LV_ALIGN_TOP_LEFT,
						  550, 60, ota_check_btn_event_cb, ui);
		{
		...
		ui->settings.ota_status_label = create_label(
			ui->settings.page_content, "Bereit", LV_ALIGN_TOP_LEFT, 10, 390);
		lv_obj_add_style(ui->settings.ota_status_label, &ui->font.very_small_20,
						 0);
	...
	case SETTINGS_PAGE_INFO:
		...
		create_text("Geraet: ESP32-S3\n"
					...
					ui->settings.page_content, STYLE_TEXT_SMALL,
					LV_ALIGN_CENTER, 0, 0, ui);
		break;
	default:
		break;
	}
...
static void backlight_save(ui_main_menu_t *ui) { main_settings_save(ui); }
static void backlight_update_auto_labels(ui_main_menu_t *ui) {
	char buf[16];
	snprintf(buf, sizeof(buf), "Min: %d%%",
			 ui->settings.switch_.backlight_auto_min_pct);
	lv_label_set_text(ui->settings.backlight_min_label, buf);
	snprintf(buf, sizeof(buf), "Max: %d%%",
			 ui->settings.switch_.backlight_auto_max_pct);
	lv_label_set_text(ui->settings.backlight_max_label, buf);
}

static void backlight_update_visibility(ui_main_menu_t *ui) {
	uint8_t mode = ui->settings.switch_.backlight_mode;
	bool manual = (mode == BACKLIGHT_MODE_MANUELL);
	bool autom = (mode == BACKLIGHT_MODE_AUTO);
	set_visible(ui->settings.backlight_slider, manual);
	set_visible(ui->settings.backlight_pct_label, manual);
	set_visible(ui->settings.backlight_min_slider, autom);
	set_visible(ui->settings.backlight_max_slider, autom);
	set_visible(ui->settings.backlight_min_label, autom);
	set_visible(ui->settings.backlight_max_label, autom);
}
...
void ui_create_settigs_display(ui_main_menu_t *ui) {
	...
	lv_obj_add_event_cb(ui->settings.backlight_btnmatrix,
						backlight_btnmatrix_event_cb, LV_EVENT_VALUE_CHANGED,
						ui);
	...
	// --- Auto Min/Max слайдеры (рядом) ---
	ui->settings.backlight_min_slider =
		lv_slider_create(ui->settings.page_content);
	lv_obj_set_size(ui->settings.backlight_min_slider, (W / 2) - 60, 30);
	lv_obj_align(ui->settings.backlight_min_slider, LV_ALIGN_TOP_LEFT, 40, 240);
	lv_slider_set_range(ui->settings.backlight_min_slider, 5, 100);
	lv_obj_add_event_cb(ui->settings.backlight_min_slider,
						backlight_min_slider_event_cb, LV_EVENT_VALUE_CHANGED,
						ui);

	ui->settings.backlight_min_label =
		create_label(ui->settings.page_content, "", LV_ALIGN_TOP_LEFT, 40, 274);
	lv_obj_add_style(ui->settings.backlight_min_label, &ui->font.very_small_20,
					 0);

	ui->settings.backlight_max_slider =
		lv_slider_create(ui->settings.page_content);
	lv_obj_set_size(ui->settings.backlight_max_slider, (W / 2) - 60, 30);
	lv_obj_align(ui->settings.backlight_max_slider, LV_ALIGN_TOP_LEFT,
				 (W / 2) + 40, 240);
	lv_slider_set_range(ui->settings.backlight_max_slider, 5, 100);
	lv_obj_add_event_cb(ui->settings.backlight_max_slider,
						backlight_max_slider_event_cb, LV_EVENT_VALUE_CHANGED,
						ui);

	ui->settings.backlight_max_label = create_label(
		ui->settings.page_content, "", LV_ALIGN_TOP_LEFT, (W / 2) + 40, 274);
	lv_obj_add_style(ui->settings.backlight_max_label, &ui->font.very_small_20,
					 0);

	// --- Standby ---
	create_text("Display Mode:", ui->settings.page_content, STYLE_TEXT_SMALL,
	...
	lv_obj_add_style(ui->settings.standby_desc_label, &ui->font.very_small_20,
					 0);
	...
	static const char *bl_descs[] = {
		...
		"Helligkeit Tag (Max) und Nacht (Min) einstellen.",
	...
	char bl_buf[8];
	snprintf(bl_buf, sizeof(bl_buf), "%d%%",
			 ui->settings.switch_.backlight_manual_pct);
	lv_label_set_text(ui->settings.backlight_pct_label, bl_buf);

	lv_slider_set_value(ui->settings.backlight_slider,
						ui->settings.switch_.backlight_manual_pct, LV_ANIM_OFF);
	lv_slider_set_value(ui->settings.backlight_min_slider,
						ui->settings.switch_.backlight_auto_min_pct,
						LV_ANIM_OFF);
	lv_slider_set_value(ui->settings.backlight_max_slider,
						ui->settings.switch_.backlight_auto_max_pct,
						LV_ANIM_OFF);
	backlight_update_auto_labels(ui);
	backlight_update_visibility(ui);

	lv_btnmatrix_set_btn_ctrl(ui->settings.standby_btnmatrix,
							  ui->settings.switch_.display_mode,
							  LV_BTNMATRIX_CTRL_CHECKED);

	static const char *standby_descs[] = {"Immer eingeschaltet.",
										  "Aus nach 180 Sek.",
...
void backlight_btnmatrix_event_cb(lv_event_t *e) {
	...
	uint8_t pct;
	if (btn_id == BACKLIGHT_MODE_MANUELL) {
		pct = ui->settings.switch_.backlight_manual_pct;
		snprintf(buf, sizeof(buf), "%d%%", pct);
		lv_label_set_text(ui->settings.backlight_pct_label, buf);
	} else if (btn_id == BACKLIGHT_MODE_AUTO) {
		pct =
			backlight_get_auto_pct(ui->settings.switch_.backlight_auto_min_pct,
								   ui->settings.switch_.backlight_auto_max_pct);
		backlight_update_auto_labels(ui);
	} else {
		pct = backlight_get_zeitplan_pct();
	}
	backlight_set(pct);
...
void backlight_min_slider_event_cb(lv_event_t *e) {
	ui_main_menu_t *ui = (ui_main_menu_t *)lv_event_get_user_data(e);
	uint8_t min =
		(uint8_t)lv_slider_get_value(ui->settings.backlight_min_slider);
	uint8_t max = ui->settings.switch_.backlight_auto_max_pct;

	if (min > max) { // Min толкает Max вверх
		max = min;
		ui->settings.switch_.backlight_auto_max_pct = max;
		lv_slider_set_value(ui->settings.backlight_max_slider, max,
							LV_ANIM_OFF);
	}
	ui->settings.switch_.backlight_auto_min_pct = min;

	backlight_update_auto_labels(ui);
	backlight_set(backlight_get_auto_pct(min, max)); // живой отклик
	backlight_save(ui);
}
...
void backlight_max_slider_event_cb(lv_event_t *e) {
	ui_main_menu_t *ui = (ui_main_menu_t *)lv_event_get_user_data(e);
	uint8_t max =
		(uint8_t)lv_slider_get_value(ui->settings.backlight_max_slider);
	uint8_t min = ui->settings.switch_.backlight_auto_min_pct;

	if (max < min) { // Max толкает Min вниз
		min = max;
		ui->settings.switch_.backlight_auto_min_pct = min;
		lv_slider_set_value(ui->settings.backlight_min_slider, min,
							LV_ANIM_OFF);
	}
	ui->settings.switch_.backlight_auto_max_pct = max;

	backlight_update_auto_labels(ui);
	backlight_set(backlight_get_auto_pct(min, max));
	backlight_save(ui);
}
...
void ui_create_settigs_wifi(ui_main_menu_t *ui) {
	...
	for (int i = 0; i < 4; i++) {
		...
		lv_obj_set_style_bg_color(
			bar, (i < bars) ? lv_color_hex(0x4CAF50) : lv_color_hex(0x444444),
			0);
	...
	lv_obj_set_style_border_color(ui->wifi.scan_list, lv_color_hex(0x2D5A8E),
								  0);
...
void ui_create_settigs_sensors(ui_main_menu_t *ui) {
...
#else
	// ── SGP30-only Variante ─────────────────────────────────────────────────
	create_text("TVOC Luftqualitaetssensor:", p, STYLE_TEXT_SMALL,
				LV_ALIGN_TOP_LEFT, X, 60, ui);
	...
	ui->settings.co2_desc_label =
		create_label(p, "", LV_ALIGN_TOP_LEFT, X, 200);
...
void ota_start_timer_cb(lv_timer_t *t) {
	lv_timer_del(t);
	lv_obj_t *ov = lv_obj_create(lv_layer_top()); // поверх всего
	lv_obj_set_size(ov, LV_HOR_RES, LV_VER_RES);
	lv_obj_set_style_bg_color(ov, lv_color_black(), 0);
	lv_obj_set_style_bg_opa(ov, LV_OPA_COVER, 0);
	lv_obj_set_style_border_width(ov, 0, 0); // убирает "рамку"
	lv_obj_set_style_radius(ov, 0, 0);		 // убирает белые углы
	lv_obj_set_style_pad_all(ov, 0, 0);

	lv_obj_t *lbl = lv_label_create(ov);
	lv_label_set_text(lbl, "Bitte warten...");
	lv_obj_add_style(lbl, &g_ui->font.very_small_20, 0);
	lv_obj_align(lbl, LV_ALIGN_TOP_LEFT, 10, 10);
	lv_obj_set_style_text_color(lbl, lv_color_hex(0x1c1c1c), 0);
	lv_obj_set_style_text_align(lbl, LV_TEXT_ALIGN_CENTER, 0);

	time_t now = time(NULL);
	if (get_time_year() > 2024) { // check if a current year is valid
...
void ota_version_check_result_cb(ota_version_status_t status,
	...
	if (lv_obj_is_valid(g_ui->settings.ota_remote_ver_label)) {
		if (remote_version)
			lv_label_set_text(g_ui->settings.ota_remote_ver_label,
							  remote_version);
		else
	...
	case OTA_VERSION_UP_TO_DATE:
		lv_label_set_text(g_ui->settings.ota_status_label,
						  "Aktuell. Kein Update noetig.");
	...
	case OTA_VERSION_UPDATE_AVAILABLE:
		...
		{
			char date_buf[32];
			uint32_t ts = ota_last_update_load();
			if (ts == 0) {
			...
			g_ui->settings.ota_last_update_label =
				create_label(g_ui->settings.page_content, date_buf,
							 LV_ALIGN_TOP_LEFT, 300, 160);
			lv_obj_add_style(g_ui->settings.ota_last_update_label,
							 &g_ui->font.medium_32, 0);
		}
